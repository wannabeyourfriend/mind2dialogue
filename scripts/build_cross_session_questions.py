...
OUT = ROOT / "output/relabel_blind/crosssession_qa.jsonl"
...
<carried_state_from_previous_session>
{state}
</carried_state_from_previous_session>
...
- the correct answer is true ONLY because of the carried state from the previous session
...
- CRITICAL — the three distractors must be EQUALLY plausible for this persona as the correct
  ...
  have the previous-session memory, must have NO basis whatsoever to prefer one option over
...
Return strict JSON:
{{"items": [{{"question": "...", "correct": "...", "distractors": ["...", "...", "..."], "evidence_from_state": "the exact sentence(s) in the carried state that make the correct answer true"}}]}}"""
