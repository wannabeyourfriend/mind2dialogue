...
QUESTION_ANSWERING_STYLES = ["personamem_mcq", "prefeval_gen", "bigtom_tom", "lamp_cls"]
...
async def command_build_question_answering_data(args: argparse.Namespace) -> None:
    ...
    from user_simulator.question_answering import (
        QuestionAnsweringStyle, generate_for_conversation, question_answering_item_to_training_line, self_consistency_check_multiple_choice,
    )
    ...
    async def process_one(path: Path) -> None:
        ...
        for style in styles:
            ...
            if item is None:
                continue
            if args.self_consistency_check and style is QuestionAnsweringStyle.PERSONAMEM_MULTIPLE_CHOICE:
                if not await self_consistency_check_multiple_choice(item, llm):
...
_REWRITE_PROMPTS = {
    "personamem_mcq": "question_answering_rewrite_personamem",
    "prefeval_gen": "question_answering_rewrite_prefeval",
    "bigtom_tom": "question_answering_rewrite_bigtom",
    "lamp_cls": "question_answering_rewrite_lamp",
}
...
async def command_rewrite_question_answering_data(args: argparse.Namespace) -> None:
    from user_simulator.data import LLM
    from user_simulator.prompts import load_prompt

    qa_dir = Path(args.question_answering_dir).resolve()
    out_dir = Path(args.output_dir).resolve()
    out_dir.mkdir(parents=True, exist_ok=True)
...
async def command_evaluate_question_answering_models(args: argparse.Namespace) -> None:
    from user_simulator.data import LLM
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--ablation", default="full",
                    choices=["full", "no_privilege", "no_behavior", "no_state", "oracle_profile_only", "guarded", "guarded_oracle", "latent", "latent_oracle"])
    ...
    sp.add_argument("--constructor", default="lifelong",
                    help="Scenario family (lifelong|high_frequency|affective|concerning) or a full prompt name")
    sp.add_argument("--max-turns", type=int, default=12)
    sp.add_argument("--min-turns", type=int, default=3)
    sp.add_argument("--persona-ids", nargs="*", help="Filter to specific persona IDs")
    ...
    sp.add_argument("--self-consistency-check", action="store_true",
    ...
    sp = sub.add_parser("rewrite-question-answering-data", help="Rewrite version 1 items into harder, persona grounded version 2 items")
    sp.add_argument("--concurrency", type=int, default=30, help="Max concurrent LLM calls")
    sp.add_argument("--question-answering-dir", required=True, help="Directory with v1 {style}.jsonl files")
    ...
    sp.set_defaults(func=command_rewrite_question_answering_data)

    sp = sub.add_parser("evaluate-question-answering-models", help="Benchmark models on the question answering data")
    sp.add_argument("--concurrency", type=int, default=10, help="Max concurrent LLM calls")
    sp.add_argument("--question-answering-dir", default="output/question_answering/v1_demo_full", help="Directory with per-style JSONL files")
    sp.add_argument("--output-dir", default="output/evaluation/v1_demo_subset", help="Where predictions + summary go")
    sp.add_argument("--styles", nargs="+", default=QUESTION_ANSWERING_STYLES, choices=QUESTION_ANSWERING_STYLES)
    ...
    sp.set_defaults(func=command_evaluate_question_answering_models)
