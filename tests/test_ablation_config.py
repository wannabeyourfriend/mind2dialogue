...
class TestFactoryDefaults:
    ...
    @pytest.fixture(
        params=["full", "no_behavior", "no_state", "oracle_profile_only"]
    )
...
@pytest.mark.asyncio
async def test_profile_only_control_injects_behavior_without_state(monkeypatch):
    from tests.fakes import FakeLLM
    from user_simulator.data import Persona
    from user_simulator.simulator import rollout as rollout_module
    from user_simulator.simulator.behavior.library import _BEHAVIORS
    ...
    async def select_behavior(*args, **kwargs):
        return {"behavior": _BEHAVIORS["analysis"], "controller_source": "test"}

    monkeypatch.setattr(rollout_module, "_select_behavior_with_controller", select_behavior)
    llm = CapturingLLM()
    llm.queue("chat", "First assistant answer.")
    llm.queue("chat", "<|Continue Conversation|>Please clarify the tradeoff.")
    llm.queue("chat", "Second assistant answer.")
    session = await rollout_module.rollout_conversation(
        Persona(id="test_persona", summary="An engineer comparing designs."),
        "Which design should I choose?", "test_prompt", llm,
        max_turns=2, config=AblationConfig.oracle_profile_only(),
    )

    assert "<behavior_id>analysis</behavior_id>" in prompts[1]
    assert "An engineer comparing designs." in prompts[0]
    assert "User State Report" not in "\n".join(prompts)
    assert all(turn["user_state"] == "" for turn in session["user_state_trajectory"])
    assert session["behavior_trajectory"][0]["behavior_id"] == "analysis"
